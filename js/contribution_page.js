(function($){
  ...
  window.ContribPage = {
        ...
        function(i, ele){
          ...
          if(regExp.test(label)){
            if(contribType == 'recurring'){
              ContribPage.defaultPriceOption['recur'] = regExp.exec(label)[1].replace(',','');
            }else if(contribType == 'non-recurring'){
              ContribPage.defaultPriceOption['single'] = regExp.exec(label)[1].replace(',','');
            }else if(contribType == ''){
              ContribPage.defaultPriceOption['recur'] = regExp.exec(label)[1].replace(',','');
              ContribPage.defaultPriceOption['single'] = regExp.exec(label)[1].replace(',','');
            }
    ...
    prepareContribTypeForm: function(){
      ...
      $('.contrib-type-btn').append($otherInstrumentBtn);
