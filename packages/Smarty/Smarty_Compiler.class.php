...
class Smarty_Compiler extends Smarty {
  ...
  public function _compile_file($resource_name, $source_content, &$compiled_content) {
    ...
    $tag_guard = '%%%SMARTYOTG' . md5(uniqid((string)rand(), TRUE)) . '%%%';
  ...
  public function _compile_tag($template_tag) {
    ...
    // html_image is removed globally; untrusted templates cannot include resources.
    if ($tag_command === 'html_image' ||
        ($this->security && in_array($tag_command, ['include', 'include_php'], TRUE))) {
      $this->_syntax_error("(secure mode) '$tag_command' is not permitted", E_USER_ERROR, __FILE__, __LINE__);
      return '';
    }
  ...
  public function _parse_modifiers(&$output, $modifier_string) {
    ...
    for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i++) {
      ...
      if (empty($this->_plugins['modifier'][$_modifier_name])
        ...
        if (function_exists($_modifier_name)
            && (!$this->security || in_array($_modifier_name, $this->security_settings['MODIFIER_FUNCS']))) {
          $this->_plugins['modifier'][$_modifier_name] = [$_modifier_name,  NULL, NULL, FALSE];
        }
        elseif ($this->security) {
          $this->_trigger_fatal_error("[plugin] (secure mode) modifier '$_modifier_name' is not allowed", $this->_current_file, $this->_current_line_no, __FILE__, __LINE__);
          // The secure renderer handles the error and keeps the
          // unmodified value instead of dropping the whole template.
          continue;
        }
      }
