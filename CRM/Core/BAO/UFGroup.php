...
class CRM_Core_BAO_UFGroup extends CRM_Core_DAO_UFGroup {
  ...
  static function getFields($id, $register = FALSE, $action = NULL,
    ...
    if (empty($fields) && !$validGroup) {
      CRM_Core_Error::statusBounce(ts('The requested Profile (gid=%1) is disabled OR it is not configured to be used for \'Profile\' listings in its Settings OR there is no Profile with that ID OR you do not have permission to access this profile. Please contact the site administrator if you need assistance.',
  ...
  public static function getValues($cid, &$fields, &$values, $searchable = TRUE, $componentWhere = NULL, $maskType = CRM_Core_BAO_UFGroup::MASK_PRIVATE) {
    ...
    foreach ($fields as $name => $field) {
      ...
      if (isset($details->$name) || $name == 'group' || $name == 'tag') {
        ...
        elseif (in_array($name, array('email_greeting', 'postal_greeting', 'addressee'))) {
          $dname = $name . '_display';
...
";
  ...
  static function buildProfile(&$form, &$field, $mode, $contactId = NULL, $online = FALSE) {
    ...
    elseif (in_array($fieldName, array('email_greeting', 'postal_greeting', 'addressee'))) {
...
";
  ...
  function setComponentDefaults(&$fields, $componentId, $component, &$defaults) {
    ...
    foreach ($fields as $name => $field) {
      ...
      elseif (in_array($name, array('contribution_type', 'payment_instrument'))) {
