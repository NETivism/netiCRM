...
class CRM_Contribute_Form_Internal extends CRM_Core_Form {
  ...
  public function buildQuickForm() {
    ...
    if ($this->_contactId) {
      ...
      $this->addSelect('original_id', ts('Based Contribution Record'), []);
    }
    ...
    $this->addFormRule(['CRM_Contribute_Form_Internal', 'formRule'], $this);
  }
  ...
  static function formRule($fields, $files, $form) {
    ...
    if (empty($fields['contact_select_id'][1])) {
      if (!ctype_digit($fields['contact_id']) || empty($fields['contact_id'])) {
        $errors['contact_id'] = ts('Please enter a valid number for %1', [1 => ts('Contact ID')]);
      }
      else {
        $found = CRM_Core_DAO::singleValueQuery("SELECT id FROM civicrm_contact WHERE id = %1 AND is_deleted = 0", [
          1 => [$fields['contact_id'], 'Positive']
        ]);
        if (!$found) {
          $errors['contact_id'] = ts('contact does not exist: %1', [1 => $fields['contact_id']]);
        }
      }
    }
    $pages = CRM_Contribute_PseudoConstant::contributionPage(NULL, TRUE);
    if (!isset($pages[$fields['contribution_page_id']])) {
      $errors['contribution_page_id'] = ts('%1 does not exists or is empty', [1 => ts('Contribution Page').'-'.$fields['contribution_page_id']]);
    }
    return $errors;
  }
  ...
  public function getContributionDetails() {
    ...
    $dao = CRM_Core_DAO::executeQuery("SELECT id, total_amount, contribution_recur_id, MAX(receive_date) FROM civicrm_contribution WHERE contribution_recur_id > 0 AND contact_id = %1 AND is_test = 0 GROUP BY contribution_recur_id ORDER BY contribution_recur_id  DESC LIMIT 3", [
