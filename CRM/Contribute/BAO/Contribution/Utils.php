...
INNER JOIN   civicrm_contact contact ON ( contact.id = contrib.contact_id ) 
   ...
   */
  public static function checkTransactionPermission($args, $op = 'and') {
    $access = TRUE;
    ...
    if ($_GET['id']) {
      ...
      if (in_array('is_renewal_only', array_keys($membershipBlockFields))) {
        ...
        $params = array( 1 => array($_GET['id'], 'Positive'));
        ...
        while($dao->fetch()) {
          ...
          if (!empty($isRenewalOnly)) {
            ...
            if ($userId) {
              ...
              $isTest = ($_GET['action'] == 'preview') ? 1 : 0;
              ...
              if ($membershipTypeError) {
                CRM_Core_Error::fatal(ts("Oops. The membership you're trying to renew appears to be invalid. Contact your site administrator if you need assistance."));
              }
            }
            // not login user.
            else {
