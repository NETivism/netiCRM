...
class CRM_Coupon_Page_CouponBatch extends CRM_Core_Page {
  ...
  function run() {
    ...
    while($dao->fetch()) {
      $used = CRM_Core_DAO::singleValueQuery("SELECT COUNT(*) FROM civicrm_coupon_track t INNER JOIN civicrm_coupon c ON c.id = t.coupon_id WHERE c.code LIKE %1 AND t.used_date IS NOT NULL", array(
        1 => array($dao->batch_prefix.'%', 'String'),
      ));
      if (empty($used)) {
        $used = 0;
      }
