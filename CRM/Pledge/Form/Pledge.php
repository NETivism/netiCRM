...
class CRM_Pledge_Form_Pledge extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    if ($this->_id) {
      ...
      $this->_contributionID = CRM_Core_DAO::getFieldValue(
  ...
  public function setDefaultValues() {
    ...
    $pledgeStatusNames = CRM_Core_OptionGroup::values(
      'contribution_status',
      FALSE,
      FALSE,
      FALSE,
    ...
    $defaultPledgeStatus = CRM_Utils_Array::value(
      array_search('Pending', $pledgeStatusNames),
      $pledgeStatus
    );

    //assign status.
    $this->assign('status', CRM_Utils_Array::value(
      CRM_Utils_Array::value('status_id', $this->_values),
      $pledgeStatus,
      $defaultPledgeStatus
    ));
    ...
    if ($this->_honorID) {
      ...
      $honorDefault = [];
      $idParams = ['contact_id' => $this->_honorID];
  ...
  public function buildQuickForm() {
    ...
    $paneNames = ['Honoree Information' => 'Honoree',
      'Payment Reminders' => 'PaymentReminders',
    ];
    foreach ($paneNames as $name => $type) {
      $urlParams = "snippet=4&formType={$type}";
      $allPanes[$name] = ['url' => CRM_Utils_System::url('civicrm/contact/view/pledge', $urlParams),
        'open' => 'false',
        'id' => $type,
      ];
    ...
    $js = ['onblur' => "calculatedPaymentAmount( );",
    ...
    $element = &$this->add(
      'text',
      'amount',
      ts('Total Pledge Amount'),
      array_merge($attributes['pledge_amount'], $js),
      TRUE
    );
    ...
    $element = &$this->add(
      ...
      'installments',
      ts('To be paid in'),
      array_merge($attributes['installments'], $js),
      TRUE
    );
    ...
    $this->add('text', 'eachPaymentAmount', ts('each'), ['size' => 10, 'style' => "background-color:#EBECE4", 'READONLY']);
    ...
    if (CRM_Utils_Array::value('status_id', $this->_values) !=
      ...
      $this->addElement('checkbox', 'is_acknowledge', ts('Send Acknowledgment?'), NULL, ['onclick' => "return showHideByValue('is_acknowledge','','acknowledgeDate','table-row','radio',true);"]);
    }
    ...
    if (CRM_Utils_Array::value('is_acknowledge', $formValues) && $pledge->id) {
      ...
      if ($this->paymentId) {
        ...
        if (count($processors) > 0) {
          $statusMsg .= ' ' . ts("If a payment is due now, you can record <a href='%1'>a check, EFT, or cash payment for this pledge</a> OR <a href='%2'>submit a credit card payment</a>.", [1 => $contribURL, 2 => $creditURL]);
        }
        else {
