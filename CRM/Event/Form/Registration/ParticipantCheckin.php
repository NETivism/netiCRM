...
class CRM_Event_Form_Registration_ParticipantCheckin extends CRM_Event_Form_Registration {
  ...
  public function preProcess() {
    ...
    $values = [];
    CRM_Core_DAO::commonRetrieve('CRM_Event_DAO_Participant', $params, $values, ['contact_id', 'event_id', 'status_id', 'register_date']);
    ...
    $values = [];
    CRM_Core_DAO::commonRetrieve('CRM_Event_DAO_Event', $params, $values, ['title']);
    $this->_event = $values;
  ...
  public function buildQuickForm() {
    if (CRM_Core_Permission::check("edit event participants")) {
      ...
      elseif ($this->_statusId == $attendedStatusId) {
        ...
        $checkinDate = CRM_Core_DAO::singleValueQuery("SELECT activity_date_time FROM civicrm_activity WHERE activity_type_id = %1 AND subject LIKE '%Attended%' AND source_record_id = %2 ORDER BY activity_date_time DESC LIMIT 1", [
          1 => [$activityTypeId, 'Integer'],
          2 => [$this->_participantId, 'Integer'],
        ]);
