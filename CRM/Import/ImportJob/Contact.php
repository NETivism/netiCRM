...
class CRM_Import_ImportJob_Contact extends CRM_Import_ImportJob {
  ...
  public function runImport(&$form) {
    ...
    if (empty($civicrm_batch)) {
      if ($this->_totalRowCount > CRM_Import_ImportJob::BATCH_THRESHOLD) {
        ...
        $batchParams = [
          ...
          'finishCallback' => [$this, 'batchFinishCallback'], // should zip all errors
    ...
    $mapperPeroperties = ['mapperRelated' => 'mapperRelatedVal',
    ...
    if(!empty($tags) && is_array($tags)) {
      foreach ($tags as $tagName) {
        if ($existsTag[$tagName]) {
          ...
          $tagId = CRM_Core_DAO::singleValueQuery($query, [1 => [$tagName, 'String']]);
          if (empty($tagId)) {
            $tagParams = [
              'name' => $tagName,
              'title' => $tagName,
