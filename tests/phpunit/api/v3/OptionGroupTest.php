...
class api_v3_OptionGroupTest extends CiviUnitTestCase {
  ...
  */
  ...
  public function testGetOptionGroupByName() {
    $params = ['name' => 'preferred_communication_method', 'version' => 3];
  ...
  public function testGetOptionGroup() {
    $result = civicrm_api('option_group', 'get', ['version' => 3]);
    $this->assertEquals(0, $result['is_error'], 'In line ' . __LINE__);
    $this->assertGreaterThan(1, $result['count'], 'In line ' . __LINE__);
  }

  public function testGetOptionDoesNotExist() {
    $result = civicrm_api('option_group', 'get', ['name' => 'FSIGUBSFGOMUUBSFGMOOUUBSFGMOOBUFSGMOOIIB', 'version' => 3]);
    $this->assertEquals(0, $result['is_error'], 'In line ' . __LINE__);
    $this->assertEquals(0, $result['count'], 'In line ' . __LINE__);
  }
  public function testGetOptionCreateSuccess() {
    $params = ['version' => $this->_apiversion, 'sequential' => 1, 'name' => 'civicrm_event.amount.560', 'is_reserved' => 1, 'is_active' => 1, 'api.OptionValue.create' => ['label' => 'workshop', 'value' => 35, 'is_default' => 1, 'is_active' => 1, 'format.only_id' => 1]];
  ...
  public function testGetOptionCreateFailOnDuplicate() {
    ...
    $result = civicrm_api('OptionGroup', 'create', $params);
    civicrm_api('OptionGroup', 'delete', ['version' => 3, 'id' => $result1['id']]);
