...
class api_v3_AddressTest extends CiviUnitTestCase {
  ...
  public function testCreateAddressTestDefaultWithID() {
    ...
    $params['id'] = $result['id'];
    $result       = civicrm_api('address', 'create', $params);
    $address      = civicrm_api('address', 'get', ['version' => 3, 'contact_id' => $params['contact_id']]);
  ...
  public function testDeleteAddress() {
    ...
    $result = civicrm_api('address', 'delete', ['id' => $create['id'], 'version' => 3]);
    $this->documentMe($this->_params, $result, __FUNCTION__, __FILE__);
    $this->assertEquals(0, $result['is_error'], 'In line ' . __LINE__);
    $this->assertEquals(1, $result['count'], 'In line ' . __LINE__);
    $get = civicrm_api('address', 'get', [
  ...
  public function testGetAddressSort() {
    ...
    $params      = ['options' => ['sort' => 'street_address DESC'],
      'version' => $this->_apiversion,
      'sequential' => 1,
    ];
    $result = civicrm_api('Address', 'Get', ($params));
  ...
  function testGetWithCustom() {
    ...
    $getParams = ['version' => 3, 'id' => $result['id'], 'return' => ['custom']];
  ...
  public function testCreateAddressPrimaryHandlingChangeToPrimary() {
    ...
    $check = civicrm_api('address', 'getcount', [
        'version' => 3,
        'is_primary' => 1,
        'id' => $address1['id'],
      ]);
