...
class CRM_Pledge_BAO_PledgeBlockTest extends CiviUnitTestCase {
  ...
  public function testCreateAndDeletePledgeBlock() {
    ...
    $pledgeFrequencyUnit = [ 'week' => 1,
                                  'month' => 1,
                                  'year' => 1 ];

    $params = [ 'entity_id'               => $this->_contributionPageId,
  ...
  public function testRetrieveAndGetPledgeBlock() {
    ...
    // use retrieve() method
    $retrieveParams = [ 'entity_id'               => $this->_contributionPageId,
                             'entity_table'            => 'civicrm_contribution_page' ];
    $default        = [ ];
