...
test.describe.serial('Contribution Booster', () => {
    ...
    test('Donor Filtering', async () => {
        ...
        await test.step('Contribution Booster page check.', async () => {
            ...
            /* Step 2: check titles */
            await expect(page.locator('#page-title')).toHaveText('Contribution Booster');
        ...
        await test.step('Donor who donate in last N month page check.', async () => {
            ...
            /* Step 4: check search results */
            await expect(page.locator('.crm-error')).toHaveCount(0);
            await expect(page.locator('#page-title')).toHaveText('Donor who donate in last 6 month');
        ...
        await test.step('After payment failed but not retry in days page check.', async () => {
            ...
            /* Step 4: check search results */
            await expect(page.locator('.crm-error')).toHaveCount(0);
            await expect(page.locator('#page-title')).toHaveText('After payment failed but not retry in 3 days');
