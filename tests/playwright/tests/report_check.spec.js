...
test.describe.serial('Report Check', () => {
    test('Check report pages', async() => {
        ...
        for (let i=1; i<=num_url; i++){
            ...
            let full_title = url_text + ' | ' + site_name;
            await expect(page).toHaveTitle(full_title);
            console.log('Enter ', full_title , ' page.');
            ...
            if (err_message != 0){
                ...
                if (i==17){
                    ...
                    await expect(page.locator('#crm-container>div>h1')).toHaveText('Membership Detail Report');
                }else{
