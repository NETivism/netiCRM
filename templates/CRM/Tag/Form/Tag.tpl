...
<h3>{if !$hideContext}{ts}Tags{/ts}{/if}</h3>
    ...
    <ul class="tree">
        ...
        <li id="tag_{$id}">
            ...
            {if $node.children}<input name="tagList[{$id}]" id="check_{$id}" type="checkbox" {if $tagged[$id]}checked="checked"{/if}/>{/if}
            {if $node.children} <span class="hit"></span> {/if} <label for="check_{$id}" id="tagLabel_{$id}">{$node.name|escape}</label> 
            {if $node.children}
            <ul>
                {foreach from=$node.children item="subnode" key="subid"}
                    <li id="tag_{$subid}">
                        ...
                        {if $subnode.children} <span class="hit"></span> {/if} <label for="check_{$subid}" id="tagLabel_{$subid}">{$subnode.name|escape}</label> 
                        {if $subnode.children}
                        <ul>
                            {foreach from=$subnode.children item="subsubnode" key="subsubid"}
                                <li id="tag_{$subsubid}">
                                    ...
                                    <label for="check_{$subsubid}" id="tagLabel_{$subsubid}">{$subsubnode.name|escape}</label>
                                </li>
                            {/foreach} 
                        </ul>
                        {/if}
