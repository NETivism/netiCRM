...
{* Risk warning dialog for non-3D TapPay payment processors on public event pages *}
...
{/foreach}
{literal}
  var isPublicEvent = {/literal}{$isPublicEvent}{literal};
  ...
  $('[id^="_qf_Fee_upload"], [id^="_qf_Fee_next"]').on('click.tappayRisk', function(e) {
    if (!isPublicEvent) {
      return true;
    }
    var selectedNon3d = [];
