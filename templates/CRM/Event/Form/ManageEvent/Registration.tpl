...
<script type="text/javascript">
    ...
    cj('select[id^="custom_p"]').on( 'change',  function( event ) {
    ...
    cj('select[id^="additional_custom_p"]').on( 'change',  function( event ) {
        buildLinks( cj(this), cj(this).val());
    });
    ...
    cj('#registration_blocks>table').append(cj('<tr class="crm-event-manage-registration-form-block-expiration_day"><td scope="row" class="label" width="20%"><label for="expiration_day">{/literal}{ts}Pending participant expiration (days){/ts}{literal}</label></td><td><input name="expiration_day" type="text" id="expiration_day" class="four"><div class="description">{/literal}{ts}Time limit <strong>in days</strong> for confirming/finishing registration by participants with any of the pending statuses. Enter 0 (or leave empty) to disable this feature.{/ts}{ts}When the time exceed, participants who are not finishing their confirming/registration will be expired automatically.{/ts} {ts}If participant choose non-credit card payment method, the automatic expiration will extend one more day for minimal payment deadline of non-credit card payment method.{/ts}<br>{ts}Please notice that if your event needs approval or have waitlist, the calculation of expiration day <strong>based on the day they receive confirmation email</strong>. Participants needs to finish their confirmation in days you set above.{/ts} {literal}</div></td></tr>'));
