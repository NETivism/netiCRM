...
{if $eventSummary.total_events}
	...
	<table class="form-layout-compressed">
	   <tr>
		...
		<td><a href="{$newEventURL}" class="button"><span><i class="zmdi zmdi-plus-circle-o"></i>{ts}New Event{/ts}</span></a></td>
	   </tr>
	</table>
    </div>
    <h3>{ts}Event Summary{/ts}  {help id="id-event-intro"}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="{$htmlFeed}"  target="_blank" title="{ts}HTML listing of current and future public events.{/ts}"><img src="{$config->resourceBase}i/applications-internet.png" alt="{ts}HTML listing of current and future public events.{/ts}" /></a>&nbsp;&nbsp;<a href="{$rssFeed}"  target="_blank" title="{ts}Get RSS 2.0 feed for current and future public events.{/ts}"><img src="{$config->resourceBase}i/feed-icon.png" alt="{ts}Get RSS 2.0 feed for current and future public events.{/ts}" /></a></h3>
