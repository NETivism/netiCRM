          ...
          <tr id="row_{$row.id}" class="{cycle values="odd-row,even-row"} {$row.class}{if NOT $row.is_active} disabled{/if}">
            ...
            <td class="crm-event-title crm-event_{$row.id}">
              ...
              <ul class="crm-nav-menu crm-nav-links-event">
                <li><a href="{crmURL p='civicrm/event/search' q="reset=1&force=1&event=`$row.id`"}" title="{ts}Statistics{/ts}"><i class="fa fa-child"></i>{ts}Statistics{/ts}</a></li>
                <li><a href="{crmURL p='civicrm/participant/add' q="reset=1&action=add&context=standalone&eid=`$row.id`"}" title="{ts}Register New Participant{/ts}"><i class="fa fa-plus"></i>{ts}Register Event Participant{/ts}</a></li>
                <li>
