  ...
  <tr id='{$list}Rowid{$row.case_id}' class="crm-case crm-case_{$row.case_id}">
    ...
    {if $list eq 'upcoming'}
	   ...
	   {if $row.case_upcoming_activity_editable}
	     <a href="{crmURL p="civicrm/case/activity" q="reset=1&cid=`$row.contact_id`&caseid=`$row.case_id`&action=update&id=`$row.case_scheduled_activity_id`"}" title="{ts}Edit this activity.{/ts}"><span class="zmdi zmdi-edit"></span></a>
	   {/if} 
	   <br />	  
