  ...
  {if $is_combination_premium}
    ...
    <table class="form-layout-compressed">
      <tr class="crm-contribution-form-block-product_name">
        ...
        {$form.product_name.html|crmReplace:onchange:"showMinContrib();"}
        {if $contributionPageId}
        <div class="description">
          {capture assign=contributionPageLink}<a href="{crmURL p="civicrm/admin/contribute/premium" q="reset=1&action=update&id=$contributionPageId"}" target="_blank">{$contributionPageTitle}</a>{/capture}
          {ts 1=$contributionPageLink}This product option comes from fundraising page %1.{/ts}
        </div>
        {/if}
        <div class="description out-of-stock" style="display: none; color: red; font-weight: bold;">
          {ts}The selected product is out of stock. Please choose another product.{/ts}
        </div>
        </td>
      </tr>
    </table>
...
{if !$is_combination_premium}
      ...
      {literal}
          ...
          function showMinContrib() {
            var product = cj("select[name='product_name[0]']")[0];
            var product_id = cj("select[name='product_name[0]']").val();
            var selectedText = cj("select[name='product_name[0]'] option:selected").text();
            var min_amount = cj("#min_amount");

            // Check for out of stock pattern [0/number]
            var outOfStockPattern = /\[0\/[0-9]+\]/;
            if (product_id > 0 && outOfStockPattern.test(selectedText)) {
              // Show out-of-stock message
              cj("select[name='product_name[0]']").val('0'); // Reset to default selection
              cj('.description.out-of-stock').show();
              hide('premium_contri');
              min_amount.val("");
              return;
            } else if(cj("select[name='product_name[0]']").val() != '0') {
              cj('.description.out-of-stock').hide();
            }
      ...
      {literal}
          if(amount[product_id]) {
            min_amount.val(amount[product_id]);
          } else {
            min_amount.val("");
          }
    }
