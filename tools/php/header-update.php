...
foreach (array_slice($argv, 1) as $arg) {
  ...
  elseif (substr($arg, 0, 2) !== '--') {
    ...
    $fileExt = pathinfo($singleFile, PATHINFO_EXTENSION);
    if ($fileExt !== 'php' && $fileExt !== 'tpl') {
      die("Error: specified file must be a .php or .tpl file: {$arg}\n");
    }
  }
}
...
$headerTplNew = phpHeaderToTpl($headerNew);
...
function phpHeaderToTpl($phpHeader) {
  ...
  if (substr($phpHeader, -3) === ' */') {
    $phpHeader = substr($phpHeader, 0, -3) . '*}';
  }
  return $phpHeader;
}
...
foreach ($fileList as $filePath) {
  ...
  elseif ($ext === 'tpl') {
    // --- Step C: Handle TPL top license block ({* ... *}) ---
    $hasTplLicenseBlock = preg_match('/^\s*(\{\*.*?\*\})/s', $content, $tplMatches);

    if ($hasTplLicenseBlock) {
      $topComment = $tplMatches[1];
      $hasAgpl = (bool) preg_match('/Affero General Public License|AGPL/i', $topComment);
      $hasCiviCrm = stripos($topComment, 'CiviCRM LLC') !== FALSE;
      $hasNetiCrm = stripos($topComment, 'NETivism') !== FALSE
        || stripos($topComment, 'neticrm.tw') !== FALSE;
      ...
      if ($hasAgpl && $hasCiviCrm) {
        ...
        $tplLegacy = phpHeaderToTpl(trim($smarty->fetch('header-php-legacy.tpl')));
        if (trim($topComment) !== $tplLegacy) {
          $content = str_replace($topComment, $tplLegacy, $content);
        }
      }
      elseif ($hasAgpl && $hasNetiCrm && !$hasCiviCrm) {
        if (trim($topComment) !== $headerTplNew) {
          $content = str_replace($topComment, $headerTplNew, $content);
        }
      }
    }
    else {
      // No license block - insert TPL header at the beginning
      $content = $headerTplNew . "\n" . $content;
    }
  }
